...
export function GatewayOverviewPage() {
  ...
  const { dashboardUrl, config: monitoringCfg } = useGrafanaUrl();
      ...
      {monitoringCfg.enabled ? (
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <div>
              <div className="text-sm font-semibold text-fg">Monitoring</div>
              <div className="text-xs text-fg-subtle">Live metrics from Grafana · auto-refreshes every 30s</div>
            </div>
            <a
          ...
          <GrafanaPanel
            ...
            title="Lumen Gateway metrics"
            height={680}
            placeholder="Configure monitoring.dashboards.gateway in config.json to see live metrics."
          />
        </div>
      ) : (
        <Card>
          <CardBody className="space-y-2">
            <div className="text-xs font-semibold text-fg">Monitoring not configured</div>
            <div className="text-xs text-fg-muted leading-relaxed">
              Set <code className="rounded bg-bg-subtle px-1 py-0.5 font-mono">modules.monitoring.enabled = true</code> and
              point <code className="rounded bg-bg-subtle px-1 py-0.5 font-mono">baseUrl</code> at your Grafana instance
              in <code className="rounded bg-bg-subtle px-1 py-0.5 font-mono">config.json</code>.
              Use the bundled <code className="rounded bg-bg-subtle px-1 py-0.5 font-mono">docker-compose.yml</code> in
              lumen-gateway to spin up Prometheus + Grafana with the dashboard pre-provisioned.
            </div>
          </CardBody>
        </Card>
      )}
